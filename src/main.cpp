...
void fileExists(const std::string& file, string flag){
	struct stat buf;
	int nisfile = stat(file.c_str(), &buf);
	int nisdir = S_ISDIR(buf.st_mode);
	int nisreg = S_ISREG(buf.st_mode);
	string isfile;
	string isdir;
	string isreg;
	if (nisfile == 0) isfile = "True";
	if (nisfile == -1) isfile = "False";
	if (nisdir == 0) isdir = "False";
	if (nisdir == 1) isdir = "True";
	if (nisreg == 0) isreg = "False";
	if (nisreg == 1) isreg = "True";
	if ((flag=="-e") || (flag=="")){
		cout << "If file exists: " << isfile << endl;
	}
	else if (flag=="-f"){
		cout << "If file exists: " << isfile << endl;
		cout << "If a regular file: "<< isreg << endl;
	}
	else if (flag=="-d"){
		cout << "If file exists: " << isfile << endl;
		cout << "If a directory: " << isdir << endl;
	}
}
...
//------------------TEST COMMAND----------------------
				...
				if (str=="test" || str=="[") {
						...
						{					
							string flag=execArgs[1];
							if ((flag=="-e")||(flag=="-d")||(flag=="-f")){
								if (execArgs[2]==NULL) success = 0;
								else{								
									string file=execArgs[2];
									fileExists(file, flag);
									success = 1;
								}
							}
							else{
								string file=execArgs[1];
								string flag="";
								fileExists(file, flag);
								success = 1;
							}
